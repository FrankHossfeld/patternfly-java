 ...
 */
public class Expandable extends ElementBuilder<HTMLDivElement, Expandable>
    ...
    Expandable(String expandText, String collapseText) {
        super(div().css(component(expandable)).get());
        ...
        button = button().css(component(expandable, toggle))
                ...
                .on(click, e -> ceh.expand(get(), buttonElement(), contentElement()))
                .add(i().css(component(expandable, toggle, icon), fas(angleRight)))
                .add(text = span().textContent(expandText).get())
                .get();
        content = div().css(component(expandable, Constants.content)).attr(hidden, "");

        // Don't use add() it's overridden!
        element.appendChild(button);
        element.appendChild(content.get());
    }
    ...
    @Override
    public Expandable addAll(Iterable<? extends Node> elements) {
        content.addAll(elements);
        return this;
    }
